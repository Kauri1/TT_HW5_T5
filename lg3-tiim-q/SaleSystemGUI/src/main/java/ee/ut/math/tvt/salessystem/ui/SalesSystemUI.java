...
public class SalesSystemUI extends Application {
    ...
    public SalesSystemUI() {
        try {
            //dao = new HibernateSalesSystemDAO();//new InMemorySalesSystemDAO();
            dao = new InMemorySalesSystemDAO();
            shoppingCart = new ShoppingCart(dao);
            this.wm = new WarehouseManager(dao);
    ...
    private void showErrorAndQuit(Exception e) {
        Alert alert = new Alert(Alert.AlertType.ERROR, "Could not reach the database. Please run the database task (hsql) and try again.");
        alert.setHeight(300);
        alert.setOnCloseRequest(r -> System.exit(0));
        alert.showAndWait();
    }
    ...
    public void start(Stage primaryStage) throws Exception {
        ...
        stockTab.setContent(loadControls("StockTab.fxml", new StockController(dao, wm)));
