...
//                nameComboBox.getItems().add(item);
//        }
        nameComboBox.getItems().addAll(dao.findItemsLeftInStock());
        ...
        // can we expect that the warehouse state stays the same during purchase process
/*        this.nameComboBox.setOnMouseClicked(new EventHandler<MouseEvent>() {
            @Override
            public void handle(MouseEvent event) {
                nameComboBox.getItems().clear();
                nameComboBox.getItems().addAll(dao.findStockItems());
            }
        });*/

        this.nameComboBox.setOnAction(new EventHandler<ActionEvent>() {
            @Override
            public void handle(ActionEvent event) {
                fillInputsBySelectedStockItemDropDown();
            }
        });

        this.nameComboBox.setConverter(new StringConverter<StockItem>() {
            @Override
            public String toString(StockItem object) {
                return object != null ? object.getName() : "";
            }

            // This is used when typing text to combo box
            @Override
            public StockItem fromString(String  string) {
                return dao.findStockItem(string);
            }
        });
    }
    ...
    private void fillInputsBySelectedStockItem() {
        ...
        if (stockItem != null && stockItem.getQuantity() != 0) {
            nameComboBox.setValue(stockItem);
    ...
    private void fillInputsBySelectedStockItemDropDown(){
        StockItem stockItem = (StockItem) nameComboBox.getValue();
        if (stockItem != null) {
    ...
    private void resetProductField() {
        ...
        //nameField.setText("");
        //nameComboBox.setSelectionModel(null);
        priceField.setText("");
    }
}
