...
public interface SalesSystemDAO {
    List<HistoryEntry> findHistoryEntries();
    List<StockItem> findStockItems();
    List<StockItem> findItemsLeftInStock();
    ...
    void saveSoldItem(SoldItem item, Long lastHistoryEntryId);
    ...
    List<HistoryEntry> getEntriesByDate(LocalDate start, LocalDate end);

    List<HistoryEntry> getNentries(int n);

    Long getLastHistoryEntryId();
}
